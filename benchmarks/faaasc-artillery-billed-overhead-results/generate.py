...
df["sumUserDuration"] = df["sumUserDuration"].astype(float)
df["sumBillDuration"] = df["sumBillDuration"].astype(float)

df["durationRatio"] = ((df["sumBillDuration"] - df["sumUserDuration"]) / df["sumBillDuration"]) * 100

df["sumUserDuration"] = df["sumUserDuration"].round(2)
df["sumBillDuration"] = df["sumBillDuration"].round(2)
df["durationRatio"] = df["durationRatio"].round(2)

df.to_csv("assets/invocation-overhead.csv")
