...
fn main() -> Result<()> {
    ...
    Faaastime::add_to_linker(&mut linker, |state: &mut FaaastimeState| state)?;

    let mut store = Store::new(&engine, FaaastimeState {});

    let (bindings, _) = Faaastime::instantiate(&mut store, &component, &linker)?;

    let rep = 0;
    let req = wasmtime::component::Resource::<IncomingRequest>::new_own(rep);
    let res = wasmtime::component::Resource::<ResponseOutparam>::new_own(rep);

    bindings.interface0.call_handle(&mut store, req, res)?;
