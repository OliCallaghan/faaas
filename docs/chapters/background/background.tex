...
% We then defer to the additional Chapter detailing isolation mechanisms in \faas{} platforms, the performance and security characteristics of each, and a summary of current research in this area for each.
...
The I/O event loop is responsible for handling asynchronous IO operations, file system operations, network requests, and IPC in a non-blocking manner. Typically, event loops use operating system level syscalls such as \verb|epoll| and \verb|io_uring| \textbf{to efficiently yield to other processes} whilst blocked by IO. These syscalls are described in more detail in Section \ref{sec:os-level-io}.

\todo[inline]{Further discuss JavaScript's own event loop ontop of libuv and tokio.}

\section{How do \faas{} platforms execute \faaslong{}?}
In this section, we will discuss the implementation details of two major \faas{} platforms execute \faas{} functions on their public cloud, and how they handle the isolation of functions from one another.

\subsection{How does AWS Lambda execute \faaslong{}}
\todo[inline]{Introduce the AWS Lambda slotting algorithm and how it executes each invocation in its own MicroVM using Firecracker.}

\subsection{How does Google Cloud Functions execute \faaslong{}}
\todo[inline]{Introduce the Google Cloud Functions slotting algorithm and how it executes each invocation in its own container using gVisor.}

\section{How do \faas{} platforms bill \faaslong{}}
\label{sec:faas-billing-models}

The \faas{} billing model is a complex model built around a number of factors that vary between cloud providers. Despite their intricacies and differences, the prevailing common billing model, referred to in Equation \ref{eq:faas-billing-model}.
...
Fuzionize \cite{schirmerFusionizeImprovingServerless2022} uses monitoring data to dynamically fuse functions together reducing network travel time.

\todo[inline]{Discuss how existing research has reduced double billing by employing function fusion.}

\subsection{First-class support of fusion primitives on \faas{} platforms}
\label{sec:fusion-primitives}
There has been some research into this area, notably Baldini et al. \cite{baldiniServerlessTrilemmaFunction2017} introducing the serverlss trilemma, arguing that function composition needs to be a first class citizen in the design of serverless platforms to avoid the double billing problem. They utilise higher order functions to compose functions together.

\todo[inline]{Discuss how in the serverless trilemma they describe the need to have first class support for function composition.}

\subsection{Homomorphism of the call stack and the double billing problem}
\todo[inline]{Whilst this sounds fucking mental, it actually illustrates the problem correctly.}
\todo[inline]{Discuss how continuations are able to utilise tail recursion to avoid the stack needing to be persisted.}
...
\section{Overprovisioning of resources}
Freyr aims to reclaim overprovisioned serverless functions \cite{yuAcceleratingServerlessComputing2022}.
